...
* Go to Reach {{model}} rover with a browser or ReachView mobile app
...
!!! note ""
	Reach {{model}} might require a thread adapter depending on the pole's thread.
{%endif%}
...
* You can see [solution status](../../reachview/status/#solution-status), {%if model == 'RS/RS+' or model == 'M+' or model == 'Module'%}[AR ratio](../../reachview/status/#ar-validation-ratio), {%endif%}[age of differential](../../reachview/status/#age-of-differential), satellite number and [Reach {{model}} name](../../reachview/settings/#change-reach-name) in the status bar placed in the top of the page. Make sure you have Fix
