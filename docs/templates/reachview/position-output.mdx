...
{%if model == "RS2" %} 
Reach RS2 also supports 230400 baud rate.
{%endif%}

#### UART
{% if model == "M2" or model == "M+" %}
Corresponds to TTL UART on Reach {{model}}. Common way to connect to autopilot or another consumer of position data.
{% else %}
Corresponds to RS232 port on Reach {{model}} extension connector. Common way to connect to autopilot or another consumer of position data. 
{% endif %}
...
[Getting Reach coordinates on Android via Bluetooth](https://docs.emlid.com/reachrs2/common/tutorials/mock-location/).
