...
#### ERB
Used for communication to Ardupilot, [protocol description can be found by following this link](https://files.emlid.com/ERB.pdf).


**Data in any of these formats can be sent via Serial, TCP or Bluetooth.**

### Serial
Serial port connection is available through several hardware connection options. Reach {{model}} supports the following baud rates: 9600, 19200, 38400, 57600, 115200{{', 230400.' if model == 'RS2' or model == 'M2' else '.'}}

#### UART
Corresponds to {{'TTL UART on Reach ' if model == 'M2' or model == 'M+' else 'RS232 port on Reach '}}{{model}}. Common way to connect to autopilot or another consumer of position data. 
...
[Getting Reach {{mode}} coordinates on Android via Bluetooth](https://docs.emlid.com/reachrs2/common/tutorials/mock-location/).
