...
Rover track is calculated relatively to the base station so in order to get rover track with correct absolute coordinates the exact position of the base station should be known. You either need to place base station on a point with known coordinate or determine it by post-processing base against a reference station in static mode. It is better if the station is in the range of 100 km (60 miles) for RS2 and 30 km (18 miles) for RS+, but longer range might work as well.
...
* Enable L1, L2 and L7 frequencies in the Frequencies tab
{%else%}
